...
// 	std::vector< DrawArraysIndirectCommand > direct_commands;
	...
	for ( size_t i = 0; i < mObjects.size(); i++ ) {
		DrawElementsIndirectCommand cmd = {
			...
			.baseInstance = (uint32_t)i,
		};
		commands.emplace_back( cmd );
// 		DrawArraysIndirectCommand direct_cmd = {
// 			.count = mObjects[i]->verticesCount(),
// 			.instanceCount = 1024*1024,
// 			.firstVertex = verticesCount,
// 			.baseInstance = (uint32_t)i,
// 		};
// 		direct_commands.emplace_back( direct_cmd );
...
// 			textureHandles.emplace_back( 0 ); // 128-bits Stride
	...
	glBufferData( GL_ARRAY_BUFFER, sizeof(float) * 16 * mObjects.size(), mMatrixObjects, GL_DYNAMIC_DRAW );
