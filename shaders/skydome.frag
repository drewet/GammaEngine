...
{
//	ray.x *= 4.0;
	float n = PNoise3(ray * scale, 6);// + PRidged3(ray * scale, 6);
...
vec3 CloudColor(vec3 ray)
{
	...
	for(i=0; i<COLOR_SAMPLES; i++)
	{
		...
		color += vec3(exp2(-1.0 * CloudDensity2(ray))) / float(COLOR_SAMPLES);
	}

	return color * sun_color * 1.35;
}
...
*/
	...
	accum.rgb += 0.75 * clamp(distance(cam, ray) / 100000.0, 0.0, 1.0);
	return accum;
}
